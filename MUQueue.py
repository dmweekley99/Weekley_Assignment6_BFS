...
class MUQueue:
    ...
    def __str__(self):
        ...
        if values:
            ...
            return "Queue is empty."  # Fixed return statement
    ...
    def dequeue(self):
        if self.head is None:  # If the queue is empty
            raise Exception("Queue is empty.")  # Raise an exception instead
        ...
        if self.head is None:  # If the queue is now empty, reset tail to None
            self.tail = None
        self.length -= 1
        return dequeued_value  # Return the dequeued value (a Vertex object)
    ...
    def size(self):
        return self.length  # Return the size of the queue
